...
function sendError(res, err) {
    return res.status(500).end(Exception.format(err));
}

function logRequest(service, fn, req) {
    if (fn.$RPC.opt && fn.$RPC.opt.log == "disable") {
        return;
    }
    // method url body
...
function handleFail(res) {
    var err = Exception.parse(res.data);
    throw err;
}

function logCall(url, obj, method) {
    var req = obj ? "\n" + JSON.stringify(obj) : "";
    logger.info("Call %s %s%s", method.toUpperCase(), url, req);
}
...
function RPC() {
    ...
    this.delete = function(fn, path, opt) {
        var match = path.match(/:/g) || [];
        var argsLength = match.length;
        if (fn.length !== argsLength) {
            throw new Exception(RPC_ERROR, "Args Not Match", {
    ...
    this.form = function(fn, path, opt) {
        var match = path.match(/:/g) || [];
    ...
    this.json = function(fn, path, opt) {
        var match = path.match(/:/g) || [];
        var argsLength = match.length + 1;
        if (fn.length !== argsLength) {
            throw new Exception(RPC_ERROR, "Args Not Match", {
                ...
                result = result.then(function() {
                    return new Promise(function(resolve, reject) {
                        var server = app.listen(port, function(err) {
                            if (err) {
                                logger.error("[%d] Start Fail", port);
                                reject(err);
                            } else {
                                logger.info("[%d] Start Succ ..............", port);
                                resolve();
                            }
                        });
                        servers[port] = server;
                ...
                result = result.then(function() {
                    return new Promise(function(resolve, reject) {
                        server.close(function(err) {
                            if (err) {
                                logger.error("Stop On [%d] Fail", port);
                                reject(err);
                            } else {
                                logger.info("Stop On [%d] Succ ...............", port);
                                resolve();
                            }
                        });
                    });
