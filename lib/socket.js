...
function readLine(that) {
    ...
    var ret = buffer.slice(0, pos + 1).toString().match(/.*/)[0];
...
Socket.prototype.read = function(len) {
    ...
    var that = this;
    if (!len && that.avaliable() > 0) {
        return Promise.resolve(that.peek(that.avaliable()));
    } else if (that.avaliable() >= len) {
    ...
    function dataHandler(data) {
        if (!len) {
            defer.resolve(that.peek(that.avaliable()));
        } else if (that.avaliable() >= len) {
