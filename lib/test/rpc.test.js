...
describe('RPC', function() {
    ...
    it('Delete Test', function(done) {
        ...
        var service = new Service();
        rpc.server(service, "/", 10080);
        rpc.start().then(function() {
            return rpc.client(service, "/", "localhost:10080").delete(100);
        }).then(function(res) {
            gid.should.eql(100);
            return rpc.stop();
        }).catch(function(err) {
    ...
    it('Json Test', function(done) {
        ...
        rpc.server(service, "/", 10080);
        rpc.start().then(function() {
            return rpc.client(service, "/", "localhost:10080").json(gobj);
        }).then(function(res) {
            res.should.eql(gobj);
            return rpc.stop();
        }).catch(function(err) {
    ...
    it('Error Test', function(done) {
        ...
        function Service() {
            this.serve = function() {
                throw new Exception("RPC_ERROR", "Test Error", {
                    a: 1,
                    b: 2
                })
            }
        ...
        rpc.server(service, "/", 10080);
        rpc.start().then(function() {
            return rpc.client(service, "/", "localhost:10080").serve();
        }).then(function(res) {
            should(true).eql(false);
        }).catch(function(err) {
            err.name.should.eql("RPC_ERROR");
